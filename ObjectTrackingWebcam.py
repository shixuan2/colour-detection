#code adapted from Murtaza Hassan's ObjectTrackingTello.py
import cv2
import numpy as np
...
#window for adjusting HSV values for colour to track
cv2.namedWindow("HSV")
cv2.resizeWindow("HSV",640,240)
cv2.createTrackbar("HUE Min","HSV",20,179,empty)
cv2.createTrackbar("HUE Max","HSV",40,179,empty)
cv2.createTrackbar("SAT Min","HSV",170,255,empty)
cv2.createTrackbar("SAT Max","HSV",255,255,empty)
cv2.createTrackbar("VALUE Min","HSV",155,255,empty)
cv2.createTrackbar("VALUE Max","HSV",255,255,empty)

#window for adjusting other parameters
...
#contours can be thought of as the "outline" curves of objects, similar to edges
def getContours(img,imgContour):
    ...
    for cnt in contours:
        ...
        if area > areaMin:
            ...
            cx = int(x + (w / 2))  #horizontal centre of object
            cy = int(y + (h / 2))  #vertical centre of object 

            #labels and saves direction that drone should fly in
...
while True:
    ...
    myFrame = cv2.flip(frame, 1)
    img = cv2.resize(myFrame, (width, height))
    imgContour = img.copy()
    imgHsv = cv2.cvtColor(img, cv2.COLOR_BGR2HSV)

    #obtain HSV values from parameters window
    ...
    #filter out everything not of the correct colour
    ...
    #get the image contour for object tracking
